...
from ..core.models import Token, TokenStats, RequestLog
...
class TokenManager:
    ...
    async def _handle_test_exception(self, token_id: int, error: Exception) -> dict:
        ...
        if status_code is not None:
            try:
                await self.db.update_token_status_code(token_id, status_code)
            except Exception as log_error:
                print(f"Failed to update token status code: {log_error}")
            try:
                await self.db.log_request(RequestLog(
                    token_id=token_id,
                ...
                print(f"Failed to log test_token error: {log_error}")

        if status_code in (401, 403):
            try:
                await self.disable_token(token_id)
            except Exception as log_error:
                print(f"Failed to disable token {token_id}: {log_error}")
    ...
    async def test_token(self, token_id: int) -> dict:
        ...
        if not token_data:
            return {"valid": False, "message": "Token not found", "status_code": None}
        ...
        try:
            ...
            return await self._handle_test_exception(token_id, e)
