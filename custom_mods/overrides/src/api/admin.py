...
async def _delete_tokens(token_ids: Iterable[int]) -> List[int]:
...
@router.delete("/api/tokens/problematic/cleanup")
async def cleanup_problematic_tokens(token: str = Depends(verify_admin_token)):
    """Delete tokens marked with 401 or expired tokens."""
    try:
        status_ids = await db.get_token_ids_by_status_code(401)
        expired_ids = await db.get_expired_token_ids()
        token_ids: Set[int] = set(status_ids + expired_ids)
        deleted_ids = await _delete_tokens(token_ids)
        return {"success": True, "deleted": len(deleted_ids), "token_ids": deleted_ids}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@router.delete("/api/tokens/status/{status_code}/cleanup")
async def cleanup_tokens_by_status(status_code: int, token: str = Depends(verify_admin_token)):
...
async def get_call_logic_config(token: str = Depends(verify_admin_token)) -> dict:
    ...
    return {
        "success": True,
        "config": {
            "polling_mode_enabled": config_obj.polling_mode_enabled
        }
    }
...
async def update_call_logic_config(
    ...
    try:
        await db.update_call_logic_config(request.polling_mode_enabled)
        config.set_polling_mode_enabled(request.polling_mode_enabled)
        return {
            "success": True,
            "message": "Call logic configuration updated",
            "polling_mode_enabled": request.polling_mode_enabled
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
