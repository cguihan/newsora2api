...
class Database:
    ...
    async def update_token_status_code(self, token_id: int, status_code: Optional[int]):
        """Update token status code flag"""
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute("""
                UPDATE tokens SET status_code = ? WHERE id = ?
            """, (status_code, token_id))
            await db.commit()

    async def get_token_ids_by_status_code(self, status_code: int) -> List[int]:
        """Get token IDs by stored status code"""
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute("""
                SELECT id FROM tokens WHERE status_code = ?
            """, (status_code,))
            rows = await cursor.fetchall()
            return [row[0] for row in rows]

    async def get_expired_token_ids(self) -> List[int]:
        """Get token IDs that are expired based on expiry_time"""
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute("""
    ...
    async def get_token_ids_by_status(self, status_code: int) -> List[int]:
        """Get distinct token IDs from request logs by status code"""
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute("""
